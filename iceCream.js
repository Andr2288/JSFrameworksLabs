...
function calculateIceCreamPrice() {
    ...
    const sizeInput = prompt('Виберіть розмір морозива: маленький або великий:');
    if (!sizeInput) {
    ...
    const toppingsInput = prompt('Виберіть начинки (шоколад, карамель, ягоди). Введіть через кому:');
    if (!toppingsInput) {
        alert('Ви не вибрали жодної начинки.');
        return;
    }
    const toppings = toppingsInput.toLowerCase().split(',');
    let toppingsPrice = 0;
    ...
    const marshmallowInput = prompt('Чи бажаєте додати маршмелоу? (так або ні):');
    const marshmallowPrice = marshmallowInput && marshmallowInput.toLowerCase() === 'так'
        ? MARSHMALLOW_PRICE
        : 0;
